...
public:
	...
	void initialize() {
		cameraPos = glm::vec3(0.0f, 5.0f, 25.0f);
...
class Light {
public:
	glm::vec3 position{ 10.0f, 0.0f, 0.0f };     // 광원의 위치
...
public:
	...
	void make_Square_Polygon() {
		...
		object_vertice_normals.push_back(glm::vec3(0.0f, 1.0f, 0.0f));
	}
	...
	void Draw_stage_square() {
		// VAO, VBO 생성
		unsigned int objectVAO, objectVBOvertex, objectVBOvertexnormal{};
...
GLvoid drawScene()
{
	...
	bottom_stage.Draw_stage_square();
	light_object.Draw_light_cube();
...
void initialize() {
	...
	// 바닥타일
	bottom_stage.make_Square_Polygon();
	bottom_stage.location = { 0.0f, -2.2f, -7.0f };
	bottom_stage.setScale({ 15.0f, 15.0f, 15.0f });
	bottom_stage.setRotation(90);
	bottom_stage.make_Color_random();

	light_object.initialize_Object("cube.obj", { 5.0f, 0.0f, 0.0f });
	light_object.color = { 1.0f, 1.0f, 0.0f };
	light_object.setScale({ 0.2f, 0.2f, 0.2f });
