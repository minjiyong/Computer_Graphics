		...
		if (timer) {
			...
			if (MOVE_J) {
				if (robot[0].direction == 1) {
					robot[0].location.y += 0.06f;
					if (collision_check()) robot[0].location.y -= 0.06f;
...
void initialize() {
	...
	stage.make_Cube_Polygon();
	stage.location = { 0.0f, 0.0f, 0.0f };
	stage.setScale({ 5.0f, 5.0f, 5.0f });
	stage.make_Color_random();
	...
	bottom_stage.location = { 0.0f, -2.5f, -2.5f };
	bottom_stage.setScale({ 5.0f, 5.0f, 5.0f });
	...
	Object temp_obj;
	// 장애물 2개
	temp_obj.initialize_Object("cube.obj", { 1.0f, -2.0f, 0.2f });
	temp_obj.setScale({ 0.2f, 0.2f, 0.2f });
	temp_obj.setRotation(90);
	object.push_back(temp_obj);

	temp_obj.initialize_Object("cube.obj", { -1.3f, -2.0f, -1.0f });
	temp_obj.setScale({ 0.2f, 0.2f, 0.2f });
	temp_obj.setRotation(90);
	object.push_back(temp_obj);
